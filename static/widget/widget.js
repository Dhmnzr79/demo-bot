...
function applyWidgetTheme(shellEl, theme) {
  ...
  const brandHover = String(theme.brandHover || "").trim();
  const brandDeep = String(theme.brandDeep || "").trim();
  const brandLight = String(theme.brandLight || "").trim();
  const brandSoft = String(theme.brandSoft || "").trim();
  const accent = String(theme.accent || "").trim();
  if (brand) shellEl.style.setProperty("--clinic-brand", brand);
  if (brandHover) shellEl.style.setProperty("--clinic-brand-hover", brandHover);
  if (brandDeep) shellEl.style.setProperty("--clinic-brand-deep", brandDeep);
  if (brandLight) shellEl.style.setProperty("--clinic-brand-light", brandLight);
  if (brandSoft) shellEl.style.setProperty("--clinic-brand-soft", brandSoft);
  if (accent) shellEl.style.setProperty("--clinic-accent", accent);
  const rgb = _hexToRgb(brand);
  if (rgb) {
    const [r, g, b] = rgb;
    shellEl.style.setProperty("--clinic-border-accent", `rgba(${r}, ${g}, ${b}, 0.42)`);
    shellEl.style.setProperty("--clinic-bg-subtle", `rgba(${r}, ${g}, ${b}, 0.06)`);
    shellEl.style.setProperty("--clinic-chip-border", `rgba(${r}, ${g}, ${b}, 0.14)`);
    shellEl.style.setProperty("--clinic-bubble-user", `rgba(${r}, ${g}, ${b}, 0.12)`);
    shellEl.style.setProperty("--clinic-shadow", `0 8px 32px rgba(${r}, ${g}, ${b}, 0.14)`);
    shellEl.style.setProperty("--clinic-shadow-soft", `0 4px 16px rgba(${r}, ${g}, ${b}, 0.1)`);
    shellEl.style.setProperty(
      "--clinic-shadow-panel",
      `0 24px 60px rgba(${r}, ${g}, ${b}, 0.16), 0 8px 24px rgba(${r}, ${g}, ${b}, 0.08)`
    );
    shellEl.style.setProperty(
      "--clinic-surface-inset",
      `inset 0 1px 0 rgba(255, 255, 255, 0.82), 0 10px 30px rgba(${r}, ${g}, ${b}, 0.08)`
    );
    shellEl.style.setProperty(
      "--clinic-focus",
      `0 0 0 2px rgba(255, 255, 255, 0.95), 0 0 0 4px rgba(${r}, ${g}, ${b}, 0.32)`
    );
    if (brandLight && brandDeep) {
      shellEl.style.setProperty(
        "--clinic-gradient-primary",
        `linear-gradient(165deg, ${brandLight} 0%, ${brand} 52%, ${brandDeep} 100%)`
      );
      shellEl.style.setProperty(
        "--clinic-gradient-send",
        `linear-gradient(165deg, ${brandLight} 0%, ${brand} 58%, ${brandDeep} 100%)`
      );
    }
  }
}
